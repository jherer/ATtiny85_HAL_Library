...
bool hal_gpio_read_pin(GPIO_t gpio_id) {
    debug_print_hex("gpio read pin", gpio_id);
    uint8_t pin_num = GET_PIN(gpio_id);
    uint8_t port_num = GET_PORT(gpio_id);
